...
def pressures_dynamic_elastance(inputs, parameters):
    ...
    # parameters
    volume_LW = inputs["volume_LW"]
    volume_alv = inputs["volume_alveoli"]
    volume_lungs = volume_LW + volume_alv
    g_LW = parameters["g_LW"]
    g_alv = parameters["g_alveoli"]
    g_TH = parameters["g_thorax"]

    # calculations
    P_elastance_LW   = elastic_pressure(volume_LW, g_LW)
...
airflow_model = Model(
    ...
    state_components=["volume_LW", "volume_alveoli"],
    inputs=inputs,
    parameters=parameters_airflow,
    initial_state={"volume_LW": 0.7, "volume_alveoli": 2.8}
)

if __name__ == "__main__":

    result = airflow_model.run_simulation(
        time=20,
        inputs={
    ...
    V_LW  = result["volume_LW"]                   # volume luchtwegen
