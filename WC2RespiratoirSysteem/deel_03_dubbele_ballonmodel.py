...
def input_pressure_as_squarewave(time, inputs):
    ...
    return ((square_wave + 1) / 2 * inputs["pressure_amplitude_lungs"])
...
def dynamics(state, inputs, parameters):
    ...
    pressure_elastance_lungs  = parameters["elastance_lungs"]\
                                * (state["volume"] - parameters["volume_rest_lungs"])
    pressure_elastance_thorax = parameters["elastance_thorax"]\
                                * (state["volume"] - parameters["volume_rest_thorax"])
    pressure_alveoli          = pressure_elastance_lungs + pressure_elastance_thorax


    flow = (inputs["pressure_lungs"] 
            - (pressure_elastance_lungs + pressure_elastance_thorax)
            ) / parameters["resistance_lungs"]

    return {"dvolume": flow,
            "pressure_elastance_lungs": pressure_elastance_lungs,
...
result = ballon_model.run_simulation(
                                    time=20,
                                    initial_state={"volume": 3.5}
)
result[["pressure_lungs", "dvolume", "volume"]].plot(subplots=True)
...
# --- Subplot 2: Debiet ---
axs[1].plot(result.index, result["dvolume"])
axs[1].set_ylabel("Debiet (L/s)")
axs[1].set_title("Luchtdebiet")
axs[1].grid()

# --- Subplot 3: Volume ---
axs[2].plot(result.index, result["volume"])
